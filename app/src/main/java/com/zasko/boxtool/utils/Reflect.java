...
public class Reflect {
    ...
    public Reflect invoke(String name, Pair... wrapperArgs) throws ReflectException {
        Method method = getMethodEx(name, wrapperArgs);
        Object[] args = new Object[wrapperArgs.length];
        for(int i = 0; i < wrapperArgs.length; i++) {
            args[i] = wrapperArgs[i].first;
        }
    ...
    public Method getMethodEx(String name, Pair... wrapperArgs) throws ReflectException {
        Class<?>[] types = new Class[wrapperArgs.length];
        for(int i = 0; i < wrapperArgs.length; i++) {
            types[i] = (Class<?>) wrapperArgs[i].second;
        }
     ...
     * @throws ReflectException If no exact match could be found
     */
